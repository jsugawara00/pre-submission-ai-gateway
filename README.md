...
なお、現時点でも完成形とは考えていません。今後もテストを積み重ね、実際にお試しいただいた結果も踏まえながら、精度をさらに高めてまいります。
...
> **照合のページ数の目安**: 1回の照合は**合計6〜7ページ程度**を目安にご利用ください。高解像度スキャンなど画像が重い資料は、これより少ない枚数でも処理に時間がかかり、無料枠の上限（**照合あたり最大300秒**）でタイムアウトする場合があります。ページ数が多いときは、1回あたりの枚数を減らし、複数回に分けて照合してください（資料を1ファイルに結合しても分割しても処理時間は変わらないため、"結合/分割"ではなく"1回あたりのページ数"が効きます）。
...
- [x] **Phase 3**: メールからの書類取り込み（届いた資料を転送するだけで照合へ）。**実装済み・疑似検証済み**。受信Webhook（共有シークレット認証）／受信トレイ（届いた書類に「チェック対象／関係書類」を割り当て）／既存エンジン・共通レポートでの照合まで実装し、擬似受信で「受信→割当→照合→レポート」の一気通貫を確認しています。**本項目の実装（疑似検証）をもってロードマップ完了とします。**
  - ※ 実際のメール受信を有効化するには、受信用の独自ドメイン（MX設定）と受信解析サービスの接続が必要です。現時点では当方が独自ドメインを保有していないため、実メールでの稼働は独自ドメイン取得後となります（アプリ側の取込み・照合機能は実装・検証済みです）。
...
本システムに関するご質問、また実際にお試しいただきたい方へのアクセスコード発行など、以下までお気軽にご連絡ください。お試しをご希望の旨をお知らせいただければ、アクセスコードを発行いたします。

📧 `jumpdevelop00@gmail.com` （左記アドレスをコピーしてご連絡ください）

> ※ お試し用のアクセスコードは、照合のたびにAI（API）の利用料が発生するため、誠に勝手ながら **照合2回まで** とさせていただいております。個人で運用しているデモのため、何卒ご理解いただけますと幸いです。

## ライセンス・利用規約

© 2026 Jump All Rights Reserved.
