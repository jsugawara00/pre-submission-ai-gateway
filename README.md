...
- 無料枠の使用量制限を守るため、認証（企業ごとのアクセスコード）とレート制限（利用回数の上限）を設けています（現在、公開に向けて設計中）

## セットアップ

ブラウザで公開URLを開くだけ（現在、公開に向けて設計中）
