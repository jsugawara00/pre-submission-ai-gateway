...
- [x] 動作確認（全機能の通し確認。レポート画面の実データ表示確認を含む。**各種のエラー確認**＝整合した原本を起点に書類を段階的に崩したパターンでの挙動検証を含む。検出された不具合の解消をもって動作確認の完了とする）— 実書類の崩し検証で挙動を確認し、検出した不具合（聞き返し確定後のheadline温存／再確認上限メッセージの重複／確定できない確認の行き止まり 等）を解消して完了
- [x] **事前チェック内のフィールド修正**: 通貨レートのフィールド追加、および運賃・保険金額のタイトルに（JPY）を追記 — 通貨レートはインボイス通貨が日本円以外のときのみ表示・入力必須（後々の円換算計算で使用予定）
