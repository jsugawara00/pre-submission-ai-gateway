...
**本番デプロイ構成**: Vercel（ホスティング）／ Vercel Blob private（PDF原本を暗号化して保管）／ **TiDB Serverless**（MySQL互換のクラウドDB）／ proxy（Next.js 16・旧 middleware）による認証（企業ごとのアクセスコード）
...
- 無料枠の使用量制限を守るため、認証（企業ごとのアクセスコード）とレート制限（利用回数の上限）を設けています

## セットアップ

ブラウザで公開URLを開くだけ。本サービスは**企業向けの限定公開**で、照合機能の利用には発行されたアクセスコードが必要です（トップページ・About は誰でも閲覧できます）。
...
- [ ] **Phase 3**: メールからの書類取り込み（届いた資料を転送するだけで照合へ）。**本項目の実装をもってロードマップ完了とする。**
  - ※ 複合機からの直接取り込みは、企業ごとの機器設定やIT運用環境への依存が大きく、汎用的な機能として提供しづらいため、今回はあえて実装対象から外しています。スキャンした資料はお手元に保存のうえ、本アプリへ添付してご利用ください。
...
- [x] **動作確認**: 崩しテストによるエラー検証に加え、高難易度のあやふやな実書類でテストを実施。精度が最大に上がるまで修正と再テストを繰り返した
- [x] **一般公開**: 企業向けの限定公開デモを公開（Vercel 本番稼働・アクセスコード制）
